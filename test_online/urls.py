...
urlpatterns = [
    ...
    url(r'^lesson/(?P<lesson_pk>\d+)/test/online/(?P<pk>\d+)/exercices/$', user_is_professor(TestDetailView.as_view(template_name="professor/lesson/test/online/exercices.haml")), name='lesson_test_online_exercices'),
    url(r'^lesson/(?P<lesson_pk>\d+)/test/online/(?P<test_pk>\d+)/(?P<test_exercice_pk>\d+)/change/$', user_is_professor(views.lesson_test_online_change_exercice), name='lesson_test_online_change_exercice'),
